                        ...
                        {movie.credits.cast.slice(0, 4).map((person: any) => (
                            ...
                            >
                                <div className="h-14 w-14 rounded-2xl overflow-hidden border-2 border-white/5 group-hover:border-accent/40 transition-all shrink-0">
                                    <img 
                                        src={person.profile_path ? `https://image.tmdb.org/t/p/w200${person.profile_path}` : "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=200"} 
                                        alt={person.name}
                                        className="h-full w-full object-cover group-hover:scale-110 transition-transform"
                                    />
                                </div>
                                <div>
                                    <p className="font-black italic uppercase text-sm tracking-tight group-hover:text-accent transition-colors">{person.name}</p>
